...
async def update_data(ctx):
    xp = await get_text_xp(ctx.guild.name, ctx.author.id)
    amount = 0
    if xp is None:
        amount = 5
        await update_xp_text(ctx.guild.name, ctx.author.id, amount)
    else:
        amount = 5 + xp
        await update_xp_text(ctx.guild.name, ctx.author.id, amount)
...
class Levelsystem(commands.Cog):
    ...
    async def on_message(self, ctx, ):
        ...
        xp = await get_text_xp(ctx.guild.name, ctx.author.id)
        amount = 0
        lvl_start = await get_lvl_text(ctx.guild.name, ctx.author.id)
        lvl_end = int(float(str(xp)) ** (1 / 4))
        if lvl_start is None:
            await update_text_lvl(ctx.guild.name, ctx.author.id)
        if lvl_start < lvl_end:
            await update_text_lvl(ctx.guild.name, ctx.author.id, lvl_end)
